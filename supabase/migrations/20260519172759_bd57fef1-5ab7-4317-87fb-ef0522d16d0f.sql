...
CREATE OR REPLACE FUNCTION public.get_asset_price_series(p_market_id uuid)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path = public
AS $$
  SELECT COALESCE(jsonb_agg(jsonb_build_object('d', observed_on, 'c', close) ORDER BY observed_on), '[]'::jsonb)
  FROM price_history
  WHERE market_id = p_market_id;
$$;

GRANT EXECUTE ON FUNCTION public.get_asset_cot_series(uuid) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_asset_price_series(uuid) TO anon, authenticated;

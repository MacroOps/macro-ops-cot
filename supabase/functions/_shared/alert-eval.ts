...
export function collectFires(alerts: AlertEvalRow[], now = Date.now()) {
  const snap = buildSnap();
  const events: AlertFireEvent[] = [];
  ...
  for (const a of alerts) {
    ...
    switch (a.operator) {
      case "gte":
        triggered = s.value >= th;
        break;
      case "lte":
        triggered = s.value <= th;
        break;
      case "crosses_above":
        triggered = s.prev < th && s.value >= th;
        break;
      case "crosses_below":
        triggered = s.prev > th && s.value <= th;
        break;
    ...
    events.push({
      alert_id: a.id,
      user_id: a.user_id,
      outseta_person_uid: a.outseta_person_uid,
      indicator_value: s.value,
      percentile: s.percentile,
      message,
    });
    updates.push({ id: a.id, last_fired_at: new Date(now).toISOString(), last_value: s.value });
  }
